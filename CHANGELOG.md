...
## Features
- **Codebuff**: spend Codebuff/Freebuff's free daily credits through 9router (`codebuff`, alias `cb`/`freebuff`). Device-code login, one session-hour per account (shared across concurrent requests, refunded on idle or model switch), agent-run registration, and CLI-envelope stamping — the upstream refuses anything that doesn't look like its own CLI. Six free models; responses are OpenAI-shaped, so no translator is involved. Session pinned to another model (409 `model_locked`) is recovered by releasing the held hour first. Multi-account safe: each login mints its own device fingerprint.
